...
The considerations are that FASTQ names must be inside the universal format of FASTQ files and 
that the sample name can not contain "_" in it. In this case FASTQ names have to be changes 
(for example replace "_" by "-", though "_" is already the universal separator for the rest of the fields."""
...
def merge(sample_name_read_dict, trim_dict, merged_folder, trimming, pairs_raw, d, pairs_trimmed):
    ...
    for sample in sample_name_read_dict:
        ...
        ## define trimmed name
        n1 = '_001-trimmed.{}'.format(extension)
        n2 = '-trimmed.{}'.format(extension)
        if len(os.path.basename(trim_dict[sample]['merged_name']).split('_')) <= 1:
            trim_dict[sample]['trimmed_name'] = os.path.basename(trim_dict[sample]['merged_name'].replace(('.' \
                                                                                                           + extension), n1))
        else:
            trim_dict[sample]['trimmed_name'] = os.path.basename(trim_dict[sample]['merged_name'].replace(('.' \
                                                                                                           + extension), n2))
        ...
        if len(sample.split('_')) > 1:
            if '1' in sample.split('_')[1]:
                ...
                s2 = (os.path.basename(d[ori_fastq_path]['mate'])).replace(('.' + extension), n2)
...
def fastq_dictionary(n, merged_folder):
    ...
    for fastq_path in n:
        ...
        d[fastq_path] = { 'path' : '', 'compressed' : '', 'name_without_extension' : '',
                          'extension': '', 'sample_name' : '', 'pair' : '', 'mate' : '',
                          'read' : '', 'lane' : '', 'barcode' : '', 'bam_name' : ''}
